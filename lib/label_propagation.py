...
class LabelPropagationClassifier(FeatureClassifier):
#     """
...
def train_label_propagation(train_exs: List[str], dev_exs: List[str], multipro_embs: MultiProtoTypeEmbeddings, feature_norms: FeatureNorms, args) -> LabelPropagationClassifier:
    ...
    #for i in range(0, 10):
        ...
        print("running label prop for feature: ", feature)
        ...
        """
        label_prop_model = LabelPropagation(kernel='knn', tol=0.01, n_neighbors=5)

        #print(len(X))
        #print(len(this_feature_for_all_words))

        res = label_prop_model.fit(X, this_feature_for_all_words)
    ...
    model  = LabelPropagationClassifier(trained_models, multipro_embs, feature_norms)
